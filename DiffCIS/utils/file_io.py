...
class StableDiffusionHandler(PathHandler):
    """
    Resolve anything that's hosted under DiffCIS's namespace.
    """
    ...
    def local_model_zoo(self):
        return os.environ.get("DiffCIS_MODEL_ZOO", "")

    def _get_local_path(self, path, **kwargs):
        name = path[len(self.PREFIX) :]
        assert name in self.URLS, f"{name} is not a valid DiffCIS model from {self.URLS.keys()}!"
        path = self.URLS[name]
